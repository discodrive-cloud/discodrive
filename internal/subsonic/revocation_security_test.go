...
func TestCredentialsRevokedOnPasswordChange(t *testing.T) {
	...
	check := func(want bool) {
		t.Helper()
		r := httptest.NewRequest("GET", "/?u="+testEmail+"&p="+testPassword, nil)
	...
	if err := h.q.SetMusicCredentials(ctx, db.SetMusicCredentialsParams{UserID: user.ID, PasswordCipher: settings.PasswordCipher, ApiKey: settings.ApiKey, TokenVersion: user.TokenVersion}); err != nil {
		t.Fatal(err)
	}
	check(false)
	// Explicitly issuing credentials from the new session restores the integration.
	if err := h.q.SetMusicCredentials(ctx, db.SetMusicCredentialsParams{UserID: user.ID, PasswordCipher: settings.PasswordCipher, ApiKey: settings.ApiKey, TokenVersion: updated.TokenVersion}); err != nil {
		t.Fatal(err)
	}
	check(true)
}
